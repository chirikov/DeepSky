...
function addpart()
{
	...
	{
		$sql = mysqli_query($db, "select id from ".$t1." where id = '$_COOKIE[ngpe_id]' limit 1");
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				$active=1;
				$sql = mysqli_query ($db, "insert into ".$t12." (vote, type, active) values ('$theme', '$type', '$active');");
				$id = mysql_insert_id();
				if (empty($id))
				$errkod=3;
				else
				{
					if (!empty($ans1))
					$sql = mysqli_query ($db, "insert into ".$t13." (vid, ans) values ('$id', '$ans1');");
					if (!empty($ans2))
					$sql2 = mysqli_query ($db, "insert into ".$t13." (vid, ans) values ('$id', '$ans2');");
					if (!empty($ans3))
					mysqli_query ($db, "insert into ".$t13." (vid, ans) values ('$id', '$ans3');");
					if (!empty($ans4))
					mysqli_query ($db, "insert into ".$t13." (vid, ans) values ('$id', '$ans4');");
					if (!empty($ans5))
					mysqli_query ($db, "insert into ".$t13." (vid, ans) values ('$id', '$ans5');");
					if (!empty($ans6))
					mysqli_query ($db, "insert into ".$t13." (vid, ans) values ('$id', '$ans6');");
					if (!empty($ans7))
					mysqli_query ($db, "insert into ".$t13." (vid, ans) values ('$id', '$ans7');");
					if (!empty($ans8))
					mysqli_query ($db, "insert into ".$t13." (vid, ans) values ('$id', '$ans8');");
					if (!empty($ans9))
					mysqli_query ($db, "insert into ".$t13." (vid, ans) values ('$id', '$ans9');");
					if (!empty($ans10))
					mysqli_query ($db, "insert into ".$t13." (vid, ans) values ('$id', '$ans10');");
					if (!$sql or !$sql2)
					$errkod=3;
				}
			}
	...
	else
	{
		...
		if (!check_if_vote($vid))
		{
			if (is_array($ans))
			{
				foreach ($ans as $a)
				{
					mysqli_query ($db, "update ".$t13." set numvotes=numvotes+1 where id = '$a'");
				}
			}
			else
			mysqli_query ($db, "update ".$t13." set numvotes=numvotes+1 where id = '$ans'");
		}
		else 	$errkod=2;
	}
	if (!empty($errkod))
...
function showresult($id)
{
	...
	$sql = mysqli_query ($db, "select vote from ".$t12." where id = '$id' limit 1");
	if (mysql_num_rows($sql)!="1")
	$errkod=1;
	else
	{
		$row = mysqli_fetch_array($sql);
		$sql = mysqli_query($db, "select ans, numvotes from ".$t13." where vid  = '$id'");
				while ($row2 = mysqli_fetch_array($sql))
		{
			$summ +=$row2[numvotes];
		}		
...
{
	global $t12;
	$sql = mysqli_query ($db, "select id, vote from ".$t12." where active='1' order by id desc");
	while ($row = mysqli_fetch_array($sql))
	{
	...
	if (!empty($_COOKIE[ngpe_id]))
	{
		if (check_pravo($_COOKIE[ngpe_id], "votesedit"))
		{
			if (empty($id))
				...
				mysqli_query ($db, "update ".$t12." set active='0' where id = '$id' limit 1");
...
function showarchive()
{
	...
	if (check_pravo($_COOKIE[ngpe_id], "votesedit"))
		{
			$sql = mysqli_query ($db, "select id, vote from ".$t12." where active='0' order by id desc");
	...
	if (!empty($_COOKIE[ngpe_id]))
	{
		if (check_pravo($_COOKIE[ngpe_id], "votesedit"))
		{
			if (empty($id))
				...
				mysqli_query ($db, "update ".$t12." set active='1' where id = '$id' limit 1");
